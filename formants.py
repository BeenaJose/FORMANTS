'''
	<formants.py> is to load the estimation and calculate the formants in wav file
...
def predict_from_times(wav_filename, preds_filename, begin, end,features_filename):
    tmp_features_filename = tempfile._get_default_tempdir() + "/" + next(tempfile._get_candidate_names()) + ".txt"
    #print tmp_features_filename
    #print wav_filename+"	"+preds_filename
    #print preds_filename
    if begin > 0.0 or end > 0.0:
        temp, quasi, lpc = features.create_features(wav_filename, tmp_features_filename,features_filename, begin, end)
        easy_call("th load_estimation_model.lua " + tmp_features_filename + ' ' + preds_filename)
        return lpc, quasi
    else:
        features.create_features(wav_filename, tmp_features_filename, features_filename)
